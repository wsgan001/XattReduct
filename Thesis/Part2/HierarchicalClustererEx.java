...
*/
public class HierarchicalClustererEx extends AbstractClusterer implements OptionHandler, CapabilitiesHandler, Drawable {
  ...
  public HierarchicalClustererEx(DistanceWithLabel dis){
	  this.m_DistanceFunction = dis;
  }
  ...
  public DistanceWithLabel m_DistanceFunction = null;
  ...
  Node [] m_clusters;
  int [] m_nClusterNr;
  public Vector<Integer> [] initClusterID =null;
  public void setInitCluster(Vector<Integer> [] initClusterID){
  ...
  public void buildClusterer(Instances data) throws Exception {
    ...
    //Vector<Integer> [] nClusterID = this.initClusterID;
    //int nClusters = this.initClusterID.length;
  ...
  void doLinkClustering(int nClusters, Vector<Integer>[] nClusterID, Node [] clusterNodes) {
    ...
    double [][] fDistance0 = new double[nInstances][nInstances];
    ...
    PriorityQueue<Tuple> queue = null;
    if(this.initClusterID!=null){
    //≥ı ºªØºØ¥ÿ
    int initNcluster = nInstances;
    Vector<Integer> [] initClusterID = this.initClusterID;
    

   
    for(int i=0;i<initClusterID.length;++i){
    	if(initClusterID[i].size()>1){
    			 
    		int base = initClusterID[i].elementAt(0);
        	for(int j=1;j<initClusterID[i].size();++j){
        		int des = initClusterID[i].elementAt(j);
        		merge(base, des, fDistance0[base][des], fDistance0[base][des], nClusterID, clusterNodes);
        		initNcluster--;
        	}
    	}
    	if(initNcluster<=m_nNumClusters)
    		break;
    }
    	
    nClusters = initNcluster;
    queue = new PriorityQueue<Tuple>(nClusters*nClusters/2, new TupleComparator());
    for (int i = 0; i < nInstances; i++) {
    	for (int j = i+1; j < nInstances; j++) {
        if (nClusterID[i].size()!=0&&nClusterID[j].size()!=0) {
 
          double fDistance = getDistance(fDistance0, nClusterID[i], nClusterID[j]);
          queue.add(new Tuple(fDistance, i, j, nClusterID[i].size(), nClusterID[j].size()));
        }
      }
    }
    }
    else
    {
    	queue = new PriorityQueue<Tuple>(nClusters*nClusters/2, new TupleComparator());
    	 for (int i = 0; i < nClusters; i++) 
    	      for (int j = i+1; j < nClusters; j++) 
    	        queue.add(new Tuple(fDistance0[j][i], i, j, 1, 1));
  ...
  double getDistance0(Vector<Integer> cluster1, Vector<Integer> cluster2) {
    ...
    case MEAN:
      ...
      fBestDist = m_DistanceFunction.distance(instance1, instance2,cluster1.elementAt(0),cluster2.elementAt(0));
