...
public class SimpleKModes 
  ...
  protected double[] moveCentroid(int centroidIndex, Instances members, boolean updateClusterInfo){
    ...
    for (int j = 0; j < members.numAttributes(); j++) {						
      ...
      vals[j] = this.meanOrMode(members,j);
  ...
  public  double meanOrMode(Instances data, int attIndex) {

	    double result, found;
	    int [] counts;
	    if (data.attribute(attIndex).isNominal()) {
	      counts = new int[data.attribute(attIndex).numValues()];
	      for (int j = 0; j < data.numInstances(); j++) {
		if (!data.instance(j).isMissing(attIndex)) {
		 
		  //counts[(int) data.instance(j).value(attIndex)] += data.attribute(attIndex).weight();
			counts[(int) data.instance(j).value(attIndex)] += data.instance(j).weight();
		}
	      }
  ...
  public void setDistanceFunction(DistanceFunction df) throws Exception {
    if(!(df instanceof EuclideanDistance) && 
       !(df instanceof DissimilarityForKmodes_Weight))
      {
