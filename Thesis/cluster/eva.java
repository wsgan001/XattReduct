...
public class eva {
	...
	public static double NMI(int[]X,int k_x,int[]Y,int k_y)
	{
		int max = Math.max(k_x, k_y);
		double h1 = entropy(X,k_x);
		double h2 = entropy(Y,k_y);
		double h12 = jointEntropy(X,Y,max);
		double mi = h1+h2-h12;
		//double ans = 2*(tmp-jointEntropy(X,Y,k))/(tmp);
		double ans = mi/Math.max(h1,h2);
