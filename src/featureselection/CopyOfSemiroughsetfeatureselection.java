...
public class CopyOfSemiroughsetfeatureselection {
	...
	public static double crosstest(Instances dataset) throws Exception {
	...
	public static evaluate_struct myevevaluate(Instances dataset, int[] arts,
		...
		if (showmsg) {
			System.out.printf("Pos:" + sum + "\n");
		}

		double entropy = getConditionalEntorpy(dataset, arts);
		if (showmsg) {
			System.out.printf("entropy:" + entropy + "\n");
		}
		evaluatevalue.entropy = entropy;

		Instances newData = datawithselectatr(dataset, arts);
		// System.out.printf(rem_selectatr.elementAt(i).size()+" ");
		double pct = crosstest(newData);
		if (showmsg) {
			System.out.printf("pct:" + pct + "\n");
		}
		evaluatevalue.pct = pct;

		newData = datawithselectatr(dataset, arts);
		double[] cluster_re = clustertest(newData);
		evaluatevalue.XB = cluster_re[0];
		evaluatevalue.Ik = cluster_re[1];
		evaluatevalue.CA = cluster_re[2];
		double appac = getappaccury(dataset, arts);
		if (showmsg) {
			System.out.printf("appac:" + appac + "\n");
		}
		evaluatevalue.appac = appac;
	...
	public static void printevaluateset(Vector<evaluate_struct> ev,
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).pos + "\t");
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).entropy + "\t");
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).pct + "\t");
			row.createCell(cellpos).setCellValue(ev.elementAt(i).pct);
			row.getCell(cellpos).setCellStyle(row.getRowStyle());
			cellpos++;
		}
		rownum=row.getRowNum()+1;
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).appac + "\t");
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).XB + "\t");
		...
		for (int i = ev.size() - 1; i > -1; i--) {
			output.write(ev.elementAt(i).Ik + "\t");
			row.createCell(cellpos).setCellValue(ev.elementAt(i).Ik);
			row.getCell(cellpos).setCellStyle(row.getRowStyle());
			cellpos++;
		}
		rownum=row.getRowNum()+1;
	...
	public static void comptest(Instances dataset) throws Exception {
		...
		for (select_atr_num = dataset.numAttributes() - 1; select_atr_num > 0; select_atr_num--) {
			...
			for (int iter = 0; iter < RAND_TEST_NUM; iter++) {
				...
				rand_pos += evaluatevalue.pos;
				rand_entropy += evaluatevalue.entropy;
				rand_pct += evaluatevalue.pct;
				rand_appac += evaluatevalue.appac;
				rand_XB += evaluatevalue.XB;
				rand_Ik += evaluatevalue.Ik;
				rand_CA += evaluatevalue.CA;
			}
			rand_pos /= RAND_TEST_NUM;
			rand_entropy /= RAND_TEST_NUM;
			rand_pct /= RAND_TEST_NUM;
			rand_appac /= RAND_TEST_NUM;
			rand_XB /= RAND_TEST_NUM;
			rand_Ik /= RAND_TEST_NUM;
			rand_CA /= RAND_TEST_NUM;
			System.out.printf("Pos:" + rand_pos + "\n");
			System.out.printf("entropy:" + rand_entropy + "\n");
			System.out.printf("pct:" + rand_pct + "\n");
			evaluatevalue = new evaluate_struct();
			evaluatevalue.pos = (int) rand_pos;
			evaluatevalue.entropy = rand_entropy;
			evaluatevalue.pct = rand_pct;
			evaluatevalue.appac = rand_appac;
			evaluatevalue.XB = rand_XB;
			evaluatevalue.Ik = rand_Ik;
			evaluatevalue.CA = rand_CA;
			evaluatevalueset.add(evaluatevalue);
	...
	public static void main(String[] args) throws Exception {
		System.out.printf("Runing begin!");
		// set-up
		RAND_TEST_NUM = 1;
		lpar = 1;
		upar = 1;
...
//		String[] datastr = {"test"};
		for (int dataset_iter = 0; dataset_iter < datastr.length; dataset_iter++) {
			...
			cursheetline = 0;
			String[] evalstr = { "pos", "entropy", "pct", "appac", "XB", "Ik" };
